...
const AdminLogin = () => {
  ...
  const onSubmitCredentials = async (event) => {
    ...
    try {
      ...
      setPendingUsername(responseData.username || username);
      setOtp('');
      setOtpMode(true);
      ...
      showToast(responseData.message || 'OTP sent to admin email.', 'success');
    } catch (apiError) {
  ...
  const onVerifyOtp = async (event) => {
    ...
    try {
      const payload = await verifyAdminOtp({
  ...
  const onResendOtp = async () => {
    ...
    try {
      const payload = await resendAdminOtp({ username: pendingUsername });
      const responseData = payload?.data || {};
      setOtp('');
      setExpiresInSeconds((Number(responseData.otpExpiryMinutes) || 5) * 60);
      setResendCooldown(Number(responseData.resendCooldownSeconds) || 30);
      ...
      <div className="w-full max-w-md">
        <Card
          ...
          subtitle={otpMode
            ...
            : 'Sign in to manage employees and attendance reports'}
        >
          {!otpMode ? (
